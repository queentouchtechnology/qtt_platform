"""
Task -> (provider, model) resolution, sourced from QTT AI Model instead of
the original Node gateway's static env-driven config.ts. This is the one
place this port is a genuine improvement over the original: changing which
model handles a task is a Desk UI edit, not a code deploy.
"""
...
def resolve_routing(task: str, requested_provider: str | None = None, requested_model: str | None = None) -> tuple[str, str]:
	...
	request always wins; otherwise looks up the QTT AI Model row whose
	...
	model_row = frappe.db.get_value(
		"QTT AI Model", {"default_for_task": task}, ["provider", "model_id"], as_dict=True
	)
	if not model_row:
		frappe.throw(_("No AI model configured for task '{0}'.").format(task))

	provider_enabled = frappe.db.get_value("QTT AI Provider", model_row.provider, "enabled")
...
def resolve_fallback_provider(exclude: str) -> str | None:
	...
	fallback = frappe.db.get_value("QTT AI Provider", {"is_fallback": 1, "enabled": 1}, "name")
...
def resolve_default_model_for_provider(provider: str) -> str | None:
	...
	mapping — picks any QTT AI Model row registered for that provider."""
	return frappe.db.get_value("QTT AI Model", {"provider": provider}, "model_id")
